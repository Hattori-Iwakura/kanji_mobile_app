...
void main() {
  ...
  group('AddKanjiToList', () {
    ...
    test('should add kanji to list and return updated list', () async {
      // arrange
      when(
        () => mockRepository.addKanjiToList(
      ...
      verify(
        () => mockRepository.addKanjiToList(listId: tListId, kanjiId: tKanjiId),
      ).called(1);
    ...
    test('should return NotFoundFailure when kanji does not exist', () async {
      // arrange
      const tFailure = NotFoundFailure('Kanji not found');
      when(
        () => mockRepository.addKanjiToList(
          listId: any(named: 'listId'),
          kanjiId: any(named: 'kanjiId'),
    ...
    test(
      'should return BadRequestFailure when kanji already in list',
      () async {
        // arrange
        const tFailure = BadRequestFailure('Kanji already exists in this list');
        when(
          () => mockRepository.addKanjiToList(
            listId: any(named: 'listId'),
            kanjiId: any(named: 'kanjiId'),
    ...
    test('should return ServerFailure when repository fails', () async {
      // arrange
      const tFailure = ServerFailure('Failed to add kanji to list');
      when(
        () => mockRepository.addKanjiToList(
          listId: any(named: 'listId'),
          kanjiId: any(named: 'kanjiId'),
